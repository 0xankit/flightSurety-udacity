...
contract FlightSuretyData {
    ...
    struct Airline {
        address airAccount; // account address of airline
    ...
    struct Insurance {
        ...
        address airAccount; // account address of airline
        string airName; // name of airline
    ...
    event AirlineRegistered(
        address indexed airAccount, // account address of airline
        string airName // name of airline
    );
    ...
    event InsuranceCreditAvailable(
        address indexed airAccount, // account address of airline
        string indexed airName, // name of airline
     ...
     */
    constructor(
        address _initialairAccount,
        string memory _initialairName
    )
    public {
        contractOwner = msg.sender;
        addAirline(_initialairAccount, _initialairName);
    }
     ...
     */
    modifier requireFundedAirline(address _airAccount) {
        require(airlines[_airAccount].isFunded == true, "Airline is not funded");
        _;
    }
    ...
    requireIsCallerAuthorized {
        addAirline(_airAccount, _airName);
    }
    ...
    private {
        ...
        emit AirlineRegistered(_airAccount, _airName);
    }
    ...
    external
    payable {
        bytes32 flightKey = getFlightKey(_airAccount, _airName, _timestamp);
        airlines[_airAccount].fund = airlines[_airAccount].fund.add(msg.value);
    ...
    requireIsCallerAuthorized {
        ...
        for (uint i = 0; i < insurances[flightKey].length; i++) {
            ...
            airlines[_airAccount].fund = airlines[_airAccount].fund.sub(amountToReceive);
            emit InsuranceCredited(insureeAccount, amountToReceive);
        }
        payoutCredited[flightKey] = true;
        emit InsuranceCreditAvailable(_airAccount, _airName, _timestamp);
    }
    ...
    payable
    requireIsCallerAuthorized {
        addFund(_airAccount, msg.value);
        airlines[_airAccount].isFunded = true;
        emit AirlineFunded(_airAccount, msg.value);
    }
    ...
    private {
        airlines[_airAccount].fund = airlines[_airAccount].fund.add(_fundValue);
    }
    ...
    view
    returns(bool) {
        return airlines[_airAccount].isRegistered == true;
    }
    ...
    requireIsCallerAuthorized
    returns(bool) {
        return airlines[_airAccount].isFunded == true;
    }
    ...
    requireIsCallerAuthorized
    returns(uint256) {
        return airlines[_airAccount].fund;
    }
